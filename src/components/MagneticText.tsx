...
export function MagneticText({ 
  ...
  return (
    <div
      ...
      className={cn("relative inline-flex items-center justify-center cursor-crosshair select-none overflow-hidden", className)}
    >
      {/* Base text layer - original text */}
      <span className={cn("text-white whitespace-nowrap", textClassName)}>{text}</span>

      <div
        ref={circleRef}
        className={cn("absolute top-0 left-0 pointer-events-none rounded-full overflow-hidden flex items-center justify-center", circleBg)}
        style={{
      ...
      >
        ...
        >
          <span className={cn("text-black whitespace-nowrap", hoverTextClassName)}>
            {hoverText || text}
          </span>
        </div>
      </div>
